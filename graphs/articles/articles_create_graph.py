"""
article code analysis graphs

usage:
    python create_graph.py <input.csv>

input csv columns:
    url, date, publication, code, code_5_sub, justification, key_quote

outputs:
    - code_distribution_bar.png             : bar chart of code frequencies with percentages
    - code_distribution_cumulative.png      : bar chart with cumulative line
    - code_proportions.csv                  : table of code counts and percentages
    - code_by_year/                         : folder with per-code yearly distribution
    - category_proportions_by_year.png      : stacked bar by category (moral taint, conduct taint, etc.)
    - stigma_over_time.png                  : grouped bar of stigma vs non-stigma per year
    - stigma_proportions_by_year.png        : stacked bar of stigma vs non-stigma
    - stigma_percentage_trend.png           : line graph of stigma % over time
    - within_stigma_composition.png         : moral taint vs conduct taint breakdown
    - moral_vs_conduct_over_time.png        : grouped bar of moral vs conduct taint
    - stigma_ratio_over_time.png            : line graph of stigma:non-stigma ratio
    - articles_per_year.png                 : total article count per year
    - code5_subcategory_distribution.png    : code 5 subcategory breakdown (a, b, c, d)
    - code5_subcategory_over_time.png       : code 5 subcategories by year
...
def load_data(filepath):
    ...
    df['year'] = pd.to_datetime(df['date'], format='ISO8601', utc=True).dt.year
    return df
...
def plot_bar_chart(counts, title='Code Distribution Across All Articles', output_file='code_distribution_bar.png'):
...
def plot_cumulative_chart(counts, title='Code Distribution Across All Articles with Cumulative Count', output_file='code_distribution_cumulative.png'):
...
def plot_code_by_year(df, code_column='code', year_column='year', output_dir='code_by_year'):
    ...
    for code in codes:
        ...
        plt.ylabel('Count', fontsize=11)
        plt.title(f'Code {code} Distribution by Year', fontsize=14, fontweight='bold')
...
def plot_proportion_table(counts, output_file='code_proportions.csv'):
...
def plot_category_over_time(df, code_column='code', year_column='year', output_file='category_over_time.png'):
    # line graph showing each category's count over years
    df = df.copy()
    ...
    for category in ['Moral Taint', 'Conduct Taint', 'Contested Role', 'Low Status', 'High Status', 'Neutral']:
        if category in grouped.columns:
            values = [grouped.loc[y, category] if y in grouped.index else 0 for y in years]
            plt.plot(years, values, marker='o', linewidth=2, label=category, color=colors[category])
...
def plot_category_cumulative_over_time(df, code_column='code', year_column='year', output_file='category_cumulative_over_time.png'):
    # line graph showing cumulative count for each category over years
    df = df.copy()
    df['category'] = df[code_column].apply(categorize_code)
    df = df.dropna(subset=['category', year_column])
    ...
    for category in ['Moral Taint', 'Conduct Taint', 'Contested Role', 'Low Status', 'High Status', 'Neutral']:
        if category in grouped.columns:
            ...
            plt.plot(years, cumulative, marker='o', linewidth=2, label=category, color=colors[category])
    
    plt.xlabel('Year', fontsize=11)
    plt.ylabel('Cumulative Count', fontsize=11)
    plt.title('Cumulative Category Distribution Over Time', fontsize=14, fontweight='bold')
    plt.xticks(years, [str(int(y)) for y in years])
    plt.legend()
...
def plot_category_proportions_by_year(df, code_column='code', year_column='year', output_file='category_proportions_by_year.png'):
    ...
    for i, total in enumerate([totals.loc[y] for y in years]):
        plt.text(i, total + 10, str(total), ha='center', va='bottom', fontsize=8, color='#333333', fontweight='bold')
    
    plt.xlabel('Year', fontsize=11)
    plt.ylabel('Article Count', fontsize=11)
    plt.title('Category Proportions by Year', fontsize=14, fontweight='bold')
...
def plot_stigma_over_time(df, code_column='code', year_column='year', output_file='stigma_over_time.png'):
    ...
    plt.xlabel('Year', fontsize=11)
    plt.ylabel('Count', fontsize=11)
    plt.title('Stigma vs Non-Stigma Distribution Over Time', fontsize=14, fontweight='bold')
    plt.xticks(x, [str(int(y)) for y in years])
    plt.legend()
...
def plot_stigma_cumulative_over_time(df, code_column='code', year_column='year', output_file='stigma_cumulative_over_time.png'):
    # line graph showing cumulative stigma vs non-stigma over years
    df = df.copy()
    df['stigma'] = df[code_column].apply(categorize_stigma)
    df = df.dropna(subset=['stigma', year_column])
    ...
    for stigma in ['Stigma', 'Non-Stigma']:
        if stigma in grouped.columns:
            values = [grouped.loc[y, stigma] if y in grouped.index else 0 for y in years]
            cumulative = []
            total = 0
            for v in values:
                total += v
                cumulative.append(total)
            plt.plot(years, cumulative, marker='o', linewidth=2, label=stigma, color=colors[stigma])
    
    plt.xlabel('Year', fontsize=11)
    plt.ylabel('Cumulative Count', fontsize=11)
    plt.title('Cumulative Stigma vs Non-Stigma Distribution Over Time', fontsize=14, fontweight='bold')
    plt.xticks(years, [str(int(y)) for y in years])
    plt.legend()
...
def plot_stigma_proportions_by_year(df, code_column='code', year_column='year', output_file='stigma_proportions_by_year.png'):
    ...
    plt.xlabel('Year', fontsize=11)
    plt.ylabel('Article Count', fontsize=11)
    plt.title('Stigma vs Non-Stigma Proportions by Year', fontsize=14, fontweight='bold')
    plt.xticks(x, [str(int(y)) for y in years])
    plt.legend(loc='upper right')
...
def plot_stigma_percentage_trend(df, code_column='code', year_column='year', output_file='stigma_percentage_trend.png'):
    ...
    plt.title('Stigma Percentage Over Time', fontsize=14, fontweight='bold')
    plt.xticks(years, [str(int(y)) for y in years])
...
def plot_within_stigma_composition(df, code_column='code', year_column='year', output_file='within_stigma_composition.png'):
    # stacked bar showing moral taint vs conduct taint as % of stigma articles
    df = df.copy()
    df['category'] = df[code_column].apply(categorize_code)
    ...
    plt.xlabel('Year', fontsize=11)
    plt.ylabel('Stigma Article Count', fontsize=11)
    plt.title('Within-Stigma Composition: Moral Taint vs Conduct Taint', fontsize=14, fontweight='bold')
    plt.xticks(x, [str(int(y)) for y in years])
    plt.legend(loc='upper right')
...
def plot_moral_vs_conduct_over_time(df, code_column='code', year_column='year', output_file='moral_vs_conduct_over_time.png'):
    ...
    # add count labels
    for bar in bars1:
        plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 2, str(int(bar.get_height())), ha='center', va='bottom', fontsize=7, color='#333333')
    for bar in bars2:
        plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 2, str(int(bar.get_height())), ha='center', va='bottom', fontsize=7, color='#333333')
    
    plt.xlabel('Year', fontsize=11)
    plt.ylabel('Count', fontsize=11)
    plt.title('Moral Taint vs Conduct Taint Over Time', fontsize=14, fontweight='bold')
    plt.xticks(x, [str(int(y)) for y in years])
    plt.legend()
...
def plot_stigma_ratio_over_time(df, code_column='code', year_column='year', output_file='stigma_ratio_over_time.png'):
    ...
    plt.title('Stigma to Non-Stigma Ratio Over Time', fontsize=14, fontweight='bold')
    plt.xticks(years, [str(int(y)) for y in years])
    plt.legend()
...
def plot_articles_per_year(df, year_column='year', output_file='articles_per_year.png'):
    # bar graph showing total article count per year
    ...
    plt.xlabel('Year', fontsize=11)
    plt.ylabel('Article Count', fontsize=11)
    plt.title('Total Articles Per Year', fontsize=14, fontweight='bold')
    plt.xticks(x, [str(int(y)) for y in years])
...
def plot_code5_subcategory(df, output_file='code5_subcategory_distribution.png'):
    ...
    plt.ylabel('Count', fontsize=11)
    plt.title('Code 5 (Criminal Conduct) Subcategory Distribution', fontsize=14, fontweight='bold')
    plt.xticks(x, labels, ha='center')
...
def plot_code5_subcategory_over_time(df, year_column='year', output_file='code5_subcategory_over_time.png'):
    ...
    for i, total in enumerate(totals):
        plt.text(i, total + 1, str(int(total)), ha='center', va='bottom', fontsize=8, color='#333333', fontweight='bold')
    
    plt.xlabel('Year', fontsize=11)
    plt.ylabel('Count', fontsize=11)
    plt.title('Code 5 (Criminal Conduct) Subcategories by Year', fontsize=14, fontweight='bold')
    plt.xticks(x, [str(int(y)) for y in years])
    plt.legend(loc='upper right')
...
def main():
    if len(sys.argv) < 2:
        print("usage: python code_proportion_graphs.py <input.csv>")
        sys.exit(1)
    ...
    # plot_category_over_time(df, code_column, year_column)
    # plot_category_cumulative_over_time(df, code_column, year_column)
    plot_category_proportions_by_year(df, code_column, year_column)
    plot_stigma_over_time(df, code_column, year_column)
    # plot_stigma_cumulative_over_time(df, code_column, year_column)
    ...
    plot_articles_per_year(df, year_column)
