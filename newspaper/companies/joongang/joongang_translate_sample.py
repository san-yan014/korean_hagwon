#!/usr/bin/env python3
"""
usage: python sample_and_translate.py input.json output.json [options]
example: python sample_and_translate.py filtered_articles.json translated_sample.json
example: python sample_and_translate.py filtered_articles.json translated_sample.json -n 100
example: python sample_and_translate.py filtered_articles.json translated_sample.json --tracker my_tracker.json

samples n articles from input file and translates them using google translate
"""
import json
import pandas as pd
import random
...
def main():
    parser = argparse.ArgumentParser(description='sample and translate korean articles')
    parser.add_argument('input_file', help='input json file with articles')
    parser.add_argument('output_file', help='output json file for translated articles')
    parser.add_argument('-n', '--num-samples', type=int, default=50, help='number of articles to sample (default: 50)')
    parser.add_argument('--tracker', help='tracker file for sampled urls (default: sampled_urls_tracker.json)')
    
    args = parser.parse_args()
    
    # set default tracker file if not specified
    if args.tracker is None:
        # use same directory as output file
        output_dir = os.path.dirname(args.output_file)
        if output_dir:
            args.tracker = os.path.join(output_dir, 'sampled_urls_tracker.json')
        else:
            args.tracker = 'sampled_urls_tracker.json'
    ...
    tracker_dir = os.path.dirname(args.tracker)
    if tracker_dir:
        os.makedirs(tracker_dir, exist_ok=True)
    
    print("\n" + "=" * 80)
    print("sample and translate korean articles")
    print("=" * 80)
    print(f"input file: {args.input_file}")
    print(f"output file: {args.output_file}")
    print(f"tracker file: {args.tracker}")
    print(f"sample size: {args.num_samples}")
    
    # step 1: sample articles
    print("\nstep 1: sampling articles")
    sample_articles = sampling(args.input_file, args.tracker, n=args.num_samples)
    ...
    translated_articles = translate_articles(sample_articles, args.output_file)
